...
/** Aggregate visitor counts for the Admin dashboard. Returns null on error. */
export async function getVisitorStats(): Promise<VisitorStats | null> {
  try {
    const { data, error } = await supabase.from('visitors').select('session_id, first_seen, visit_count')
...
/** Most-viewed paths, aggregated client-side (fine at portfolio-scale traffic). */
export async function getMostViewedPaths(limit = 20): Promise<PathViews[]> {
  try {
    const { data, error } = await supabase.from('page_views').select('path').limit(5000)
...
export async function getRecentVisitors(limit = 50): Promise<Visitor[]> {
  try {
    const { data, error } = await supabase.from('visitors').select('*').order('last_seen', { ascending: false }).limit(limit)
    if (error || !data) return []
